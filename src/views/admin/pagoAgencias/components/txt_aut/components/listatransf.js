...
import FilaTransf from 'components/subComponents/Listados/SubComponentes/FilaTransf2'
import React, { useEffect, useState } from 'react'
import { Col, Container, FormGroup, Input, Label, Row, Spinner } from 'reactstrap'
import FileSaver from 'file-saver'
import formatMoney from 'Function/NumberFormat'

const titulos = ["Destino", "CBU", "Concepto", "Motivo", "Monto", ""]
...
const ListaTransf = ({
    ...
    const GenerarTxt = async () => {
        setLoading(true)
        const newList = arrayPagos.filter(item => item.importe > 0)
        await axios.post(UrlNodeServer.proveedoresDir.sub.newTxt, newList, {
